...
const ProvisionPresets = React.memo<ProvisionPresetsProps>(({ onSelect }) => {
  return (
    <div>
      <h4 style={{ color: '#374151' }} className="font-medium mb-3">📋 Provisions prédéfinies</h4>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {PRESET_PROVISIONS.map((preset) => (
          <button
            ...
            className="p-3 hover:bg-zinc-100 rounded-lg text-center text-sm border border-zinc-200 hover:border-zinc-300 transition-colors"
          >
            <div className="text-lg mb-1">{preset.icon}</div>
            <div style={{ color: '#1f2937' }} className="text-xs font-medium">{preset.name}</div>
          </button>
        ))}
      </div>
    </div>
